//
// Created by redamancyguy on 24-10-30.
//
...
public :
    ...
    explicit FLARE(WorkLoad<T> &workload, idx_t output_dim, json &result, idx_t hidden_dim = 256, idx_t layers = 2) {
        MemoryLog memory_log;
        long long memory_occupy = 0;
        TimerClock tc;
        TimerClock tc2;
        const int64_t input_dim = workload.dim;
        int batch_size = 0.09 * 1024. * 1024.;
        batch_size /= 7;
        batch_size = 768;
        float max_lr = 5e-4;
        const int64_t d_size = workload.size;
#ifdef lambda
    const int64_t q_size = workload.kNN_query_size();
#endif
        ...
        net = new NN(input_dim, output_dim);
...
#ifdef lambda
    torch::Tensor Q = torch::zeros({q_size, input_dim});
    for (idx_t i = 0; i < q_size; ++i) {
        Q.narrow(0, i, 1).copy_(workload.get_kNN_query(i).first);
    }
#endif
...
#ifdef lambda
        torch::Tensor Q_batch = Q.index_select(0, torch::randint(0, q_size, {batch_size}, torch::kInt64)).to(GPU_DEVICE);
        Q_batch = scalar(Q_batch, mean, std_var);
        auto qy_batch = net->forward(Q_batch.detach());
#endif

            auto original_dis = L2_mean_dis(X0_batch.detach(), X1_batch.detach());
            // auto original_dis = L2_mean_dis_(X0_batch.detach(), X1_batch.detach());
            auto weight = 1/(original_dis.detach() + 1);
            auto projected_dis = L2_mean_dis(y0_batch, y1_batch);
            // auto projected_dis = L2_mean_dis_(y0_batch, y1_batch);
            auto error = original_dis - projected_dis;
            error = error * error;
            auto loss_data = (error).mean();
            // auto loss_data = (weight.detach() * error).mean();
#ifdef lambda
        auto original_dis_query = torch::pairwise_distance(X0_batch.detach() ,Q_batch.detach())/std::sqrt(input_dim);
        auto projected_dis_query = torch::pairwise_distance(y0_batch ,y1_batch)/std::sqrt(input_dim);
        auto loss_query = loss_f(original_dis_query, projected_dis_query);
        auto loss = (1 - lambda) * loss_data + lambda * loss_query;
#else
            auto &loss = loss_data;
#endif
            optimizer.zero_grad();
