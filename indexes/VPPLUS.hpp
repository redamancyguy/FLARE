...
template<class T>
class VPPLUS final : public Index<T> {
public:
    enum constructType {
        KMeans = 0,
        KMeansShrink = 1,
...
private:
    ...
    public:
        ...
        // std::vector<idx_t> angle_data_size;
...
public:
    ...
    explicit VPPLUS(WorkLoad<T> &workload): Index<T>(workload), dim(workload.dim) {
...
public:
    ...
    void AkNN(const num_t *center, const idx_t k,const num_t c,
        ...
        while (!nodePQ.empty()) {
            ...
            if (dataPQ.size() >= k && !nodePQ.empty() && c * nodePQ.top().first >= dataPQ.top().first) { break; }
        }
        while (!dataPQ.empty()) {
...
template<class T>
idx_t VPPLUS<T>::node_count = 0;
template<class T>
idx_t VPPLUS<T>::leaf_size = DEFAULT_LEAF_SIZE;
template<class T>
idx_t VPPLUS<T>::fanout = DEFAULT_FANOUT;
template<class T>
idx_t VPPLUS<T>::angle_fanout = 1.5 * DEFAULT_FANOUT;
template<class T>
bool VPPLUS<T>::using_angle = true;
template<class T>
idx_t VPPLUS<T>::angle_k_means_it = 2;
template<class T>
typename VPPLUS<T>::constructType VPPLUS<T>::construct_type = constructType::KMeans;
